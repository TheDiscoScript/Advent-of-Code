...
const get_rows_1 = require("../utilities/get-rows");
...
const parseRow = (row) => {
    ...
    for (let i = 0; i < gameSet.length; i++) {
        ...
        for (let j = 0; j < splitPairs.length; j++) {
            ...
            if (!parsed[gameNumber][colour])
                parsed[gameNumber][colour] = number;
            else if (parsed[gameNumber][colour] < number)
                parsed[gameNumber][colour] = number;
            continue;
        }
    }
...
const decidePossibility = (parsed) => {
    const parsedValues = Object.values(parsed)[0];
    for (let i = 0; i < colours.length; i++) {
        const colour = colours[i];
        if (parsedValues[colour] > testQubes[colour])
            return false;
        continue;
    }
    return true;
...
const run = (input) => {
    const rows = (0, get_rows_1.getRows)(input);
...
const runInputData = () => {
    const inputData = (0, file_reader_1.readInputFile)('day2', 'input.txt');
    if (!inputData)
