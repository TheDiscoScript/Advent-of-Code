...
const getCalibrationStrings = (input: string): string[] => {
	return input.split('\n');
};
...
const runPart1 = (input: string): number => {
...
const runInputData = () => {
	const inputData = readInputFile('day1', 'input.txt');
	if (!inputData) {
...
//14+
//76 = 281
const getCalibrationValueModified = (input: string): string => {
	const dictionary = {
		one: '1',
		two: '2',
		three: '3',
		...
		if (backAdder.length >= 3) {
			for (let key of Object.keys(dictionary)) {
				if (backAdder.includes(key)) {
					left = dictionary[key as keyof typeof dictionary];
					break;
				}
			}
		}
		...
		if (forwardAdder.length >= 3) {
			for (let key of Object.keys(dictionary)) {
				if (forwardAdder.includes(key)) {
					right = dictionary[key as keyof typeof dictionary];
					break;
				}
			}
		}
...
const runPart2 = (input: string): number => {
	const calibrationStrings = getCalibrationStrings(input);
